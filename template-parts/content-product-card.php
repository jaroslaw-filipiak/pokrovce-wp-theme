...
<div class="product-card group">
    <div class="relative overflow-hidden rounded-lg bg-[#F6F7FB] aspect-square mb-4 p-2 flex items-center justify-center">
        <!-- Clickable overlay - important for mobile -->
        <a href="<?php echo esc_url( $product_link ); ?>" 
           class="absolute inset-0 z-10" 
           aria-label="<?php echo esc_attr( $product_title ); ?>">
            <span class="sr-only"><?php echo esc_html( $product_title ); ?></span>
        </a>
        ...
        <span class="absolute top-4 left-4 bg-[#E67E22] text-white text-xs font-medium px-2 py-1 rounded z-20">
            <?php esc_html_e( 'Promocja', 'pokrovce' ); ?>
        </span>
        <?php endif; ?>

        <!-- Action buttons - z-20 to be above the clickable overlay -->
        <div class="product-card-actions absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0 z-20">
