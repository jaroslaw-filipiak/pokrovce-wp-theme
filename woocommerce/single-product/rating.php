...
<div class="woocommerce-product-rating flex flex-wrap items-center gap-5.5 mb-4.5">
	...
	<div class="flex items-center gap-2.5">
		<div class="flex items-center gap-1">
			<?php for ( $i = 1; $i <= 5; $i++ ) : ?>
				<svg width="18" height="18" viewBox="0 0 18 18" fill="currentColor" xmlns="http://www.w3.org/2000/svg" class="<?php echo $i <= round( $average ) ? 'fill-yellow' : 'fill-gray-4'; ?>">
		...
		<span>( <?php echo esc_html( $review_count ); ?> <?php esc_html_e( 'customer reviews', 'pokrovce' ); ?> )</span>
	</div>
	...
	<div class="flex items-center gap-1.5">
		<?php if ( 'instock' === $stock_status ) : ?>
			...
			<span class="text-green"><?php esc_html_e( 'In Stock', 'pokrovce' ); ?></span>
		<?php elseif ( 'outofstock' === $stock_status ) : ?>
			<span class="text-red"><?php esc_html_e( 'Out of Stock', 'pokrovce' ); ?></span>
		<?php else : ?>
			<span class="text-yellow-dark"><?php esc_html_e( 'On Backorder', 'pokrovce' ); ?></span>
		<?php endif; ?>
	</div>
</div>
